...
/**
 * GET /api/schedule
 * Возвращает фильмы с сеансами на указанный день (today/tomorrow)
 */
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // SQL-запрос для получения фильмов и их сеансов на указанную дату
    // backend/routes/schedule.js
    const [results] = await sequelize.query(`
        SELECT 
            f.film_id,
            f.title,
            g.genre_name,  -- Получаем имя жанра из таблицы genres
            f.duration_min,
            COALESCE(AVG(r.rating), 0.0) as avg_rating,  -- Средний рейтинг из reviews
            f.poster_url,
            f.description,
            s.screening_id,
            s.start_time,
        ...
        JOIN genres g ON f.genre_id = g.genre_id  -- Новый JOIN для genre_name
        LEFT JOIN reviews r ON f.film_id = r.film_id  -- LEFT JOIN, потому что не все фильмы имеют отзывы
